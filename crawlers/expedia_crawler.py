...
class ExpediaCrawler(BaseCrawler):
    """Expedia crawler implementation"""
    ...
    async def search_properties(self, hotel_name: str, city: str, checkin_date: str, 
        ...
        """Search for properties on Expedia"""
        results = []
        ...
        try:
            ...
            self.logger.info(f"Searching Expedia for: {search_query}")
            
            # Get search results page
            soup = await self.get_page_soup(search_url)
            if not soup:
                self.logger.warning("Failed to get Expedia search page")
                return results
            ...
            self.logger.info(f"Found {len(listings)} listings on Expedia")
            ...
            self.logger.error(f"Error searching Expedia: {str(e)}")
    ...
    async def _extract_listing_data(self, listing_element, base_url: str) -> Optional[SearchResult]:
        ...
        try:
            ...
            name_elem = listing_element.find('h3', {'data-stid': 'content-hotel-title'})
            if not name_elem:
            ...
            # Extract price
            price_elem = listing_element.find('span', {'data-stid': 'price-lockup-text'})
            if not price_elem:
            ...
            # Extract rating
            rating_elem = listing_element.find('span', {'data-stid': 'review-score'})
            if not rating_elem:
            ...
            if link_elem and link_elem.get('href'):
                url = f"https://www.expedia.com{link_elem['href']}"
            ...
            return SearchResult(
                ...
                price=price,
                currency="USD",  # Expedia typically shows USD
                availability=True,
                url=url,
                rating=rating,
