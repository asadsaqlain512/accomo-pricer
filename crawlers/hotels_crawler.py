...
class HotelsCrawler(BaseCrawler):
    """Hotels.com crawler implementation"""
    ...
    async def search_properties(self, hotel_name: str, city: str, checkin_date: str, 
        ...
        """Search for properties on Hotels.com"""
        results = []
        ...
        try:
            ...
            # Hotels.com search URL structure
            ...
            params = {
                ...
                'q-rooms': 1,
                'q-room-0-adults': kwargs.get('guests', 1),
                'q-room-0-children': 0
            }
            
            search_url += '?' + urlencode(params)
            
            self.logger.info(f"Searching Hotels.com for: {search_query}")
            
            # Get search results page
            soup = await self.get_page_soup(search_url)
            if not soup:
                self.logger.warning("Failed to get Hotels.com search page")
                return results
            
            # Extract property listings
            listings = soup.find_all('div', {'data-hotel-id': True})
            if not listings:
                # Try alternative selectors
                listings = soup.find_all('div', class_=re.compile(r'hotel-item'))
            
            self.logger.info(f"Found {len(listings)} listings on Hotels.com")
            ...
            self.logger.error(f"Error searching Hotels.com: {str(e)}")
    ...
    async def _extract_listing_data(self, listing_element, base_url: str) -> Optional[SearchResult]:
        ...
        try:
            ...
            name_elem = listing_element.find('h3', class_='p-name')
            if not name_elem:
            ...
            # Extract rating
            rating_elem = listing_element.find('span', class_='rating')
            if not rating_elem:
            ...
            review_elem = listing_element.find('span', class_='review-count')
            if not review_elem:
            ...
            if link_elem and link_elem.get('href'):
                url = f"https://www.hotels.com{link_elem['href']}"
            ...
            return SearchResult(
                ...
                price=price,
                currency="USD",  # Hotels.com typically shows USD
                availability=True,
                url=url,
                rating=rating,
